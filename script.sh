...
csvsort -c 5 -r tmdb-movies.csv | tail -n +2 | awk -F, '{print $6}' | head -1

csvgrep -c 5 -r '^[1-9].*' tmdb-movies.csv| csvsort -c 5 -r | awk -F, '{print $6}' | tail -1
...
csvsql --query "select *, revenue - budget as profit from movie" --table movie tmdb-movies.csv | csvsort -c 22 -r| tail -n+2| awk -F, '{print $6}' | head -10 

#6.Đạo diễn nào có nhiều bộ phim nhất và diễn viên nào đóng nhiều phim nhất
awk -F, '{split($9, a,"|"); for(i in a) {if (a[i] != "") print a[i]}}' normalize-movies.csv | sort | uniq -c | sort -nr | awk '$1=""; {print }'|head -1
